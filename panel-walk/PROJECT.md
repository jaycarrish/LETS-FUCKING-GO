...
Latest confirmed scope: upgrade BOTH existing separately metered service/panel sections to 200A each. Three-phase is not required. This supersedes the earlier like-for-like 100A replacement direction. Treat this as a two-service 200A upgrade project (400A aggregate requested capacity across two meter positions), not as one 200A total service. Preserve separate metering unless SCE/City-approved service design requires a different arrangement.

Procurement direction: do not purchase utility-metering equipment solely from retail fit/price. Obtain SCE approval for the meter/service arrangement and voltage before release because the current CL200 meter markings do not establish socket/bus/service-conductor capacity, and SCE requirements govern metering. The compact Square D SC816F200PF is a current candidate only: 200A, 120/240V 1-phase CSED, semi-flush, 8 spaces/16 circuits, 22kA rating, EUSERC, factory 200A main; it is about 38.4 in high x 14.85/16.8 in wide x 3.4 in deep and therefore cannot fit wholly inside the existing 33.5 in wide x 16 in high panel opening. Two 200A services require a revised enclosure/wall layout or approved grouped service equipment, not a sideways installation.
...
Submit/confirm the SCE service-upgrade design for two 200A meter positions and verify service voltage, feed method, available fault current, meter-socket/bypass requirements, and point of delivery. In parallel, finalize the branch-circuit schedule and service-conductor route/lengths. Release the two 200A CSEDs/meter mains only after utility acceptance; then finalize conductors, grounding/bonding, hubs/tunnel kits, raceway and branch breakers. The 33.5 in x 16 in existing opening is not a constraint that current 200A CSED candidates can satisfy without wall/layout changes.
